"""
Segformer is borrowed from https://github.com/FrancescoSaverioZuppichini/SegFormer
"""
...
class SegFormer(nn.Module):
    """
    SegFormer is a network that takes in a batch of images and outputs a segmentation mask.
    Expect a mask of spatial shape image_size // 4"""
    ...
    def __init__(
        ...
        self.head = SegFormerSegmentationHead(decoder_channels, n_class, num_features=len(widths))

    def forward(self, x, upsampling=True):
        features = self.encoder(x)
        features = self.decoder(features[::-1])
        segmentation = self.head(features)
        if upsampling:
            """
            mask of spatial shape image_size // 4 witthout upsampling
            """
            h, w = x.shape[-2:]
            return F.interpolate(segmentation, (h, w), mode="nearest")
        return segmentation


# segformer = SegFormer(
#    in_chns=3, n_class=100)
